export default function SingleChatOption ({ chat }) {
  ...
  return (
    <>
      <div className={`max-w-[25vw] px-3 flex items-center font-sans py-3 ${isActive ? 'bg-[#F05454]' : ''}`}>
        {/* IMAGE */}
        <img
        ...
        <section className={`flex-1 ml-6 flex flex-col gap-0.5 `}>
          ...
          <p className={`text-gray-400 font-semibold tracking-wider line-clamp-1 ${isActive?'text-white':''}`}>
            {isGrouped ? <>
             <span className="font-bold text-gray-300">{chat?.message?.name}</span>:{" "}{chat?.message?.message}
            </>:<>
            {chat?.message}            
            </>}
          </p>
